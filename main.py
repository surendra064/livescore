...
def get_current_matches():
    ...
    soup = BeautifulSoup(page.text, 'xml')  # parsing and building xml tree out of the returned xml file
    matches = soup.find_all('description')  # description tags contain the score
    live_matches = [s.get_text() for s in matches if
                    '*' in s.get_text()]  # returns only the live matches and ignores the completed ones
    return live_matches
...
if __name__ == "__main__":
    ...
    # shows desktop notification every 30 seconds
    while True:
